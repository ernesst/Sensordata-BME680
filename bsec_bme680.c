...
#define DESTZONE "TZ=America/Mexico_City"
                  ...
                  float breath_voc_equivalent)
{
  ...
  printf("%02d-%02d-%02dT%02d:%02d:%02dZ ", tm.tm_year + 1900, tm.tm_mon,tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec); /* localtime */
  printf("%.2f %d ", iaq, iaq_accuracy);
  printf("%.2f %.2f ",
        raw_temperature,
        raw_humidity);
  printf("%.2f %.2f ", temperature, humidity);
  printf("%.2f ", pressure / 100);
  printf("%.0f %d", gas, bsec_status);
